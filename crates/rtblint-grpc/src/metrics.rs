...
///
/// rtblint benchmarks at 363µs light and 2,104µs heavy per tag, so the
...
fn metrics() -> &'static Metrics {
    METRICS.get_or_init(|| {
        ...
        let latency = HistogramVec::new(
            HistogramOpts::new(
                "rtblint_grpc_request_duration_seconds",
...
mod tests {
    ...
    fn rendering_produces_prometheus_text() {
        ...
        assert!(rendered.contains("rtblint_grpc_requests_total"));
        assert!(rendered.contains("rtblint_grpc_shed_total"));
        assert!(rendered.contains("rtblint_grpc_concurrency_limit 17"));
