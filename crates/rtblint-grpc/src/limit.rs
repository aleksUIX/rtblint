...
/// Long-lived calls this layer must not govern.
///
/// Empty, because every RPC in this service is unary. It exists anyway, with
/// this note, because the sibling vastlint server learned the hard way what
/// happens when a streaming call goes through a per-request limiter: the call
/// holds a concurrency slot for its whole lifetime, and the latency sample
/// taken when it ends is the stream's entire duration. A sixty-second stream
/// reports sixty seconds, so every stream that ends drives the limit down
/// multiplicatively until the server sheds unary callers while doing almost
/// nothing.
///
/// If a streaming RPC is ever added here, its path goes in this list and
/// admission moves to per message inside the handler. Leaving the mechanism in
/// place costs nothing and means the next person does not have to rediscover
/// the failure.
const UNGOVERNED_PATHS: [&str; 0] = [];
...
mod tests {
    ...
    async fn the_layer_sheds_with_resource_exhausted() {
        ...
        let _occupied = service.call(request("/openadtech.rtblint.v1.RtblintService/Validate"));
        assert_eq!(limiter.inflight(), 1);

        let shed = service
            .call(request("/openadtech.rtblint.v1.RtblintService/Validate"))
            .await
    ...
    /// Inherited from the sibling server, which shipped a streaming RPC through
    /// a per-request limiter and found that the latency sample taken when a
    /// stream ends is the stream's whole lifetime. This service is unary only,
    /// so the failure cannot occur here today. The test stays because it
    /// documents why [`UNGOVERNED_PATHS`] exists while it is empty, and it will
    /// fail loudly the day someone adds a long-lived RPC without adding it to
    /// that list.
    #[test]
    fn a_long_lived_call_would_poison_the_limit() {
        let limiter = limiter(config());
        let before = limiter.limit();

        // What the layer would record for a one-minute call.
        limiter.sample(Duration::from_secs(60), 0);
        assert!(
            before > limiter.limit(),
            "a sample that long drives the limit down"
        );
        ...
        assert_eq!(
            limiter.limit(),
            config().min,
            "sustained long calls would ratchet the limit to the floor, which is why any \
             long-lived RPC added here must go in UNGOVERNED_PATHS and admit per unit of work"
        );
    }

    /// Every RPC on this service is unary, so nothing is ungoverned. Asserted
    /// rather than assumed: if an RPC is added to the list, whoever adds it
    /// should have to state here that they meant to.
    #[test]
    fn every_rpc_is_currently_governed() {
        assert!(
            UNGOVERNED_PATHS.is_empty(),
            "an ungoverned path was added without updating this test: {UNGOVERNED_PATHS:?}"
        );
    }
    ...
    fn health_and_reflection_paths_are_exempt() {
        ...
        assert!(
            !EXEMPT_PREFIXES.iter().any(|prefix| {
                "/openadtech.rtblint.v1.RtblintService/Validate".starts_with(prefix)
            }),
