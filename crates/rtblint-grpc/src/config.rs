...
impl Default for Config {
    fn default() -> Self {
        Self {
            addr: "0.0.0.0:50061".parse().expect("valid default addr"),
            metrics_addr: Some("0.0.0.0:9091".parse().expect("valid default metrics addr")),
            ...
            caller_header: "x-rtblint-caller".to_string(),
            worker_threads: None,
...
impl Config {
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        ...
        let metrics_addr = match std::env::var("RTBLINT_METRICS_ADDR") {
            ...
            Ok(raw) => Some(parse("RTBLINT_METRICS_ADDR", &raw)?),
        };

        let per_second = env_parse("RTBLINT_RATE_LIMIT_RPS", defaults.rate_limit.per_second)?;
        let burst = match std::env::var("RTBLINT_RATE_LIMIT_BURST") {
            Ok(raw) => parse("RTBLINT_RATE_LIMIT_BURST", &raw)?,
        ...
        let config = Self {
            addr: match std::env::var("RTBLINT_GRPC_ADDR") {
                Ok(raw) => parse("RTBLINT_GRPC_ADDR", &raw)?,
                Err(_) => defaults.addr,
            },
            metrics_addr,
            limit: LimitConfig {
                enabled: env_flag("RTBLINT_LIMIT_ENABLED", defaults.limit.enabled)?,
                initial: env_parse("RTBLINT_LIMIT_INITIAL", defaults.limit.initial)?,
                min: env_parse("RTBLINT_LIMIT_MIN", defaults.limit.min)?,
                max: env_parse("RTBLINT_LIMIT_MAX", defaults.limit.max)?,
                target_latency: Duration::from_millis(env_parse(
                    "RTBLINT_LIMIT_TARGET_LATENCY_MS",
                    defaults.limit.target_latency.as_millis() as u64,
                )?),
                backoff_ratio: env_parse("RTBLINT_LIMIT_BACKOFF", defaults.limit.backoff_ratio)?,
            },
            rate_limit: RateLimitConfig { per_second, burst },
            max_message_bytes: env_parse("RTBLINT_MAX_MESSAGE_BYTES", defaults.max_message_bytes)?,
            caller_header: std::env::var("RTBLINT_CALLER_HEADER").unwrap_or(defaults.caller_header),
            worker_threads: match std::env::var("RTBLINT_WORKER_THREADS") {
                Ok(raw) => Some(parse("RTBLINT_WORKER_THREADS", &raw)?),
                Err(_) => defaults.worker_threads,
            },
            blocking_threads: env_parse("RTBLINT_BLOCKING_THREADS", defaults.blocking_threads)?,
        };
    ...
    fn validate(&self) -> Result<(), ConfigError> {
        ...
        if self.limit.min > self.limit.max {
            return Err(ConfigError::Invalid {
                name: "RTBLINT_LIMIT_MIN",
                reason: "must not exceed RTBLINT_LIMIT_MAX",
            });
        }

        if !(0.0..1.0).contains(&self.limit.backoff_ratio) {
            return Err(ConfigError::Invalid {
                name: "RTBLINT_LIMIT_BACKOFF",
        ...
        if self.max_message_bytes == 0 {
            return Err(ConfigError::Invalid {
                name: "RTBLINT_MAX_MESSAGE_BYTES",
...
mod tests {
    ...
    fn flags_accept_the_usual_spellings() {
        assert!(env_flag("RTBLINT_TEST_MISSING_FLAG", true).unwrap());
        assert!(!env_flag("RTBLINT_TEST_MISSING_FLAG", false).unwrap());
    }
}
